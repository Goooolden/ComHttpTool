...
+ (void)upLoadWithURL:(NSString *)url param:(NSDictionary *)params image:(UIImage *)image imageName:(NSString *)imageName keyName:(NSString *)keyName success:(HttpSuccess)success failure:(HttpFailure)failure {
    ...
    [manager POST:url parameters:params headers:nil constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        // 注意这里的需要的话设置key的值
        [formData appendPartWithFileURL:imageUrl name:keyName error:NULL];
...
+ (void)upLoadMutiWithURL:(NSString *)url param:(NSDictionary *)params imageDicArray:(NSArray *)imageDicArray keyName:(NSString *)keyName success:(HttpSuccess)success failure:(HttpFailure)failure {
    // 存放保存至沙盒中的图片路径
...
+ (void)upLoadMutiWithURL:(NSString *)url headers:(NSDictionary *)headers param:(NSDictionary *)params imageDicArray:(NSArray *)imageDicArray keyName:(NSString *)keyName success:(HttpSuccess)success failure:(HttpFailure)failure {
    // 存放保存至沙盒中的图片路径
    NSMutableArray *imageUrlArr = [NSMutableArray arrayWithCapacity:10];
    dispatch_queue_t queue =  dispatch_queue_create("cccccccccccccc", NULL);
...
+ (NSURLSessionDownloadTask *)downLoadWithUrl:(NSString *)url withName:(NSString *)name withFilePath:(NSString *)filePath downLoadPrpgrsss:(DownloadProgress)progress state:(CompletionState)completion {
    ...
    // 2、初始化
    NSURLSessionConfiguration *configuration = [NSURLSessionConfiguration defaultSessionConfiguration];
    // [NSURLSessionConfiguration backgroundSessionConfigurationWithIdentifier:url];
    ...
    NSURLSessionDownloadTask *task = [manager downloadTaskWithRequest:request progress:^(NSProgress *_Nonnull downloadProgress) {
        ...
        [fileManager createDirectoryAtPath:aFilePath withIntermediateDirectories:YES attributes:nil error:nil];
        NSURL *filePathURL = [NSURL fileURLWithPath:aFilePath];
        // NSLog(@"downloadTasks: %@  destination", manager.downloadTasks);
        // 转化为文件路径
        return [filePathURL URLByAppendingPathComponent:name];
